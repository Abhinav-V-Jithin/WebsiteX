...
def register():
    ...
    db.commit()
    print(f"from register:{'username' in session}")
    return redirect(url_for('book_store'))
...
def verify():
    ...
    session["username"] = username
    return redirect(url_for('book_store'))
@app.route("/book_store", methods=["GET"])
def book_store():
...
def search():
    ...
    if isEmpty(years) == "empty":
        is_year = False
    return render_template("search_results.html", query=query,authors=authors,titles=titles,years=years,isbns=isbns,is_isbn=is_isbn,is_year=is_year,is_title=is_title,is_author=is_author)
